...
import { getIPOApplications, withdrawIPOApplication, getIPOs, initializeAcceleratedIPOs } from "../../services/ipoService";
...
const IPOAllotmentSection = () => {
  ...
  const showFallbackAlert = (type, title, message) => {
    if (showAlert) {
  ...
  const processInstantIPO = async (applicationId, symbol) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/ipo/instant-process`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (isAllotted) {
          ...
          // AUTOMATICALLY UPDATE TO LISTED STATUS AFTER 2 SECONDS
          setTimeout(() => {
            setApplications(prev => prev.map(app => 
              app.id === applicationId 
                ? { ...app, status: 'listed' }
            ...
            console.log(`📈 ${symbol}: Automatically listed at ₹${listingPrice}, P&L: ₹${totalProfit} (${profitPercentage}%)`);
  ...
  // Process instant IPO for truly new applications only
  useEffect(() => {
    applications.forEach(app => {
      ...
      // IMPORTANT: Check if application is truly new (not processed yet)
      // Only process applications that don't have allotmentDate or listingDate
      const isTrulyNewApplication = !app.allotmentDate && !app.listingDate && (app.status === 'applied' || app.status === 'pending');
      
      if (isTrulyNewApplication) {
        console.log(`🔄 Processing NEW application: ${app.ipoSymbol}, status: ${app.status}`);
        // Process IPO instantly only for truly new applications
    ...
    try {
      ...
      if (applicationsData.length > 0) {
        try {
          ...
          const initResponse = await initializeAcceleratedIPOs(applicationsData);
  ...
  const handleWithdraw = async (applicationId, ipoName, ipoSymbol, amount, status) => {
    ...
    if (status === 'not_allotted') {
      ...
      showFallbackAlert("warning", "Cannot Withdraw", "This application cannot be withdrawn at this time.");
      return;
    }
    ...
    try {
      ...
      showFallbackAlert("success", "Withdrawal Successful", `₹${response.data.amount?.toLocaleString() || amount.toLocaleString()} has been refunded to your wallet`);

      // Refresh applications and wallet balance
      fetchApplications();
      // Also refresh wallet balance if you have a function for that
      ...
      showFallbackAlert("error", "Withdrawal Failed", error.response?.data?.message || error.response?.data?.reason || "Failed to withdraw application");
    } finally {
      setWithdrawing(null);
    }
  };
