...
const generateRealisticDates = (baseIPO, today, index) => {
  ...
  let daysFromToday;
  if (index < 25) {
    daysFromToday = 0; // First 25 IPOs open today
  } else if (index < 35) {
    daysFromToday = 1; // Next 10 IPOs open tomorrow
  } else if (index < 40) {
    daysFromToday = 2; // Next 5 IPOs open day after tomorrow
  } else {
    daysFromToday = index - 35; // Rest spread out
  }
  
  // Note: Allow IPOs to open on weekends for better user experience
...
const initializeIPOState = () => {
  ...
  // Generate realistic dates for all IPOs based on today's date
  const allIPOs = [
    ...iposOpen.map((ipo, index) => generateRealisticDates(ipo, today, index)),
    ...iposUpcoming.map((ipo, index) => generateRealisticDates(ipo, today, index + iposOpen.length))
  ];
  ...
  // Override status based on actual classification
  ipoState = { 
